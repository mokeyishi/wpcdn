...
class Password_Protection {
    ...
    public function maybe_show_login_form() {
        ...
        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
            ...
            $ip_address_whitelist_raw = ( isset( $options['password_protection_ip_whitelist'] ) ) ? explode( PHP_EOL, $options['password_protection_ip_whitelist'] ) : array();
            ...
            // Skip password protection for whitelisted IP addresses
            ...
            // Check for password in 'pass' URL parameter
            $password_in_url = ( isset( $_GET['bypass'] ) ) ? sanitize_text_field( $_GET['bypass'] ) : '';
            $stored_password = $options['password_protection_password'];
            
            // Allow to view the site if password in URL and DB matches
            if ( $password_in_url == $stored_password ) {
                if ( empty( $auth_cookie ) ) {
                    setcookie( 'asenha_password_protection', $hashed_cookie_value, $expiration, COOKIEPATH, COOKIE_DOMAIN, false, true );   
                }
                return; // Do not load login form or perform redirection to the login form
