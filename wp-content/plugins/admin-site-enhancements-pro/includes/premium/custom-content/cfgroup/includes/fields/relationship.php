...
class cfgroup_relationship extends cfgroup_field
{
    ...
    ?>
        ...
        <?php foreach ( $available_posts as $post ) : ?>
            ...
            <div rel="<?php echo $post->ID; ?>"<?php echo $class; ?> title="<?php echo esc_attr( $post->post_type ); ?>"><?php echo apply_filters( 'cfgroup_relationship_display', $post->post_title, $post->ID, $field ); ?></div>
        <?php endforeach; ?>
        </div>

        <div class="selected_posts post_list">
        <?php foreach ( $selected_posts as $post ) : ?>
            <div rel="<?php echo $post->ID; ?>"><span class="remove"></span><?php echo apply_filters( 'cfgroup_relationship_display', $post->post_title, $post->ID, $field ); ?></div>
        <?php endforeach; ?>
        </div>
        <div class="clear"></div>
    ...
    function pre_save( $value, $field = null ) {
        if ( ! empty( $value ) ) {
            ...
            // Inside a repeater, the value is $value[0]
            $value = (array) $value;
